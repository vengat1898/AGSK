...
export default function PayOnline() {
  ...
  const saveQRCodeToGallery = async () => {
    try {
      ...
      Alert.alert('Error', 'Failed to save QR code to gallery. Please try again.');
    }
  };
  ...
  const pickImage = async () => {
    try {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        mediaTypes: ImagePicker.MediaTypeOptions.Images, // Try old API first
        allowsEditing: true,
        quality: 0.8,
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Try Camera', onPress: pickImageFromCamera }
        ]
      );
    }
  };

  // Alternative: Pick image from camera
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const finalParams = {
        ...
        type,
        order_id: "[" + confirmOrder + "]",
        offer_code: coupon_code || 0,  // Use the destructured coupon_code
        discount: discount_amount || 0, // Use the destructured discount_amount
      };

      console.log("Final order params:", finalParams); // Add this for debugging
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.qrSection}>
          <Text style={[styles.label, { textAlign: 'center', fontSize: 18, fontWeight: '600', marginBottom: 15 }]}>
            Scan QR Code to Pay
          </Text>
          ...
          {loadingUpi ? (
            ...
            <>
              <TouchableOpacity onPress={handleQRCodeActions} activeOpacity={0.8}>
                <View style={{
