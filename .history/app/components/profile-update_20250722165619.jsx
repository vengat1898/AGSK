  ...
  const loadProfileData = async () => {
    try {
      ...
      if (session) {
        ...
        const response = await api.get(`/Customer/fetch_profile_by_id?customer_id=${session.id}`);
        console.log('====================================');
        ...
        if (response.data.status === 1) {
          ...
          setImage(profileData.image ? `${api.defaults.baseURL}/uploads/${profileData.image}` : null);
...
console.log('====================================');
      ...
      if (response.data.status === 1) {
        ...
        Alert.alert('Update Failed', response.data.message || 'Something went wrong');
      }
    } catch (error) {
  ...
  const getUserTypeLabel = () => {
    switch (userType) {
      case 2: return 'Hotel Name';
      case 3: return 'Catering Name';
      ...
      >
        ...
        >
          ...
          <View style={styles.formWrapper}>
            ...
            <TouchableOpacity 
              style={[styles.updateButton, (isUploading || isUpdating) && styles.buttonDisabled]} 
