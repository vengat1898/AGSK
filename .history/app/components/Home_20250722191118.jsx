 ...
 const handleAddToCartClick = async (item) => {
  if (addedToCart[item.product_id]) {
    ...
    try {
      ...
      // Find the order_id for this product (you might need to store this when adding to cart)
      // For now, let's assume you have a way to get the order_id
      // You might need to modify your addToCart response to store the order_id
      
      // Option 1: If you have order_id stored somewhere
      // const orderId = item.order_id; // This should be the order_id from cart data
      
      // Option 2: If you need to fetch cart data to get order_id
      // You might need to call a get cart API to find the order_id for this product
      
      // For now, let's call the delete API with the product info
      ...
      const response = await axios.get(
        "https://minsway.co.in/leaf/mb/Delete/delete",
        {
          params: {
            mobile: mobile,
            delete: item.product_id, // or item.order_id if you have it
          },
        }
      );
      ...
      if (response.data.success === 1) {
        ...
        Alert.alert("Error", response.data.message || "Failed to remove from cart");
      }
    } catch (error) {
      ...
      <SafeAreaView style={styles.container}>
        ...
        <SafeAreaView style={styles.footerSafeArea}>
          <View style={styles.footerNav}>
            ...
            <TouchableOpacity
              style={styles.navItem}
              onPress={() => router.push("/components/MyOrder")}
            >
            ...
            <TouchableOpacity
              style={styles.navItem}
              onPress={() => router.push("/components/profile-update")}
            >
