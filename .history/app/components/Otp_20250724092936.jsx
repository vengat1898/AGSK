...
export default function Otp() {
  ...
  const handleChange = (text, index) => {
    ...
    // Auto-focus next input if current field has value and not the last field
    if (numericText && index < 3) {
      inputs.current[index + 1]?.focus();
    }
    // If current field is empty and we're clearing, focus previous input
  ...
  const handleKeyPress = (e, index) => {
    ...
    if (e.nativeEvent.key === 'Backspace') {
      if (!otp[index] && index > 0) {
        ...
        // If current field has value, clear it and stay on current field
        const newOtp = [...otp];
        newOtp[index] = '';
        setOtp(newOtp);
      }
