...
const Enquiry = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      if (response.data.success === 1) {
        // Filter enquiries based on selected tab
        const currentTabStatus = tabs.find(tab => tab.key === selectedTab)?.status;
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "1": return "#FF8C00"; // Orange for pending
      case "2": return "#2E7D32"; // Dark green for accepted
      case "3": return "#D32F2F"; // Red for rejected
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.value}>{new Date(item.Customer_date).toLocaleDateString()}</Text>
        </View>
        
        <View style={styles.infoRow}>
          <Text style={styles.label}>🚚 Delivery Date:</Text>
          <Text style={styles.value}>{new Date(item.delivery_date).toLocaleDateString()}</Text>
        </View>
        ...
        {item.supplier_message && (
          <View style={styles.supplierMessageContainer}>
            ...
            <Text style={styles.supplierMessageText}>{item.supplier_message}</Text>
          </View>
        )}
