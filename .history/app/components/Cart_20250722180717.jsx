...
export default function Cart() {
  ...
  const { session, getUserMobile, getUserId, getUserName, getUserType } = useContext(SessionContext);
  ...
  const fetchCartData = async () => {
    try {
      ...
      console.log('🛒 Fetching cart for Mobile:', sessionMobile, 'ID:', sessionId);
  ...
  const deleteCartItem = async (orderId) => {
    try {
      ...
      const sessionMobile = getUserMobile();
  ...
  <View style={styles.card}>
    ...
    <View style={styles.cardDetails}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
        <Text style={styles.quantityText}>{`${item.count} leaf`}</Text>
