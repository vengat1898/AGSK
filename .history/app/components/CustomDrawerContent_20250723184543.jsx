...
const CustomDrawer = ({ isVisible, onClose }) => {
  ...
  const { session, clearSession, getUserName, getUserMobile, getUserType, saveSession } =
...
// In the render section, add debug logging:
...
{(() => {
  ...
  return (userTypeNumber === 2 || userTypeNumber === 3) && (
    <TouchableOpacity
          ...
          onPress: async () => {
            try {
              ...
              if (response.data.success === 1) {
                ...
                Alert.alert("Success", `Profile switched to ${newTypeLabel} successfully!`, [
                  {
                    text: "OK",
                    onPress: () => {
                      onClose();
