...
const [loadingBalance, setLoadingBalance] = useState(false);
  ...
  const { session, getUserMobile, getUserId, getUserName, getUserType ,getSecondaryMobile} =
  ...
  try {
    ...
    if (response.data.success === 1 && response.data.data) {
      ...
      const custBalance = parseFloat(response.data.data["Customer balance"]) || 0;
...
const fetchInvoiceAndUser = async () => {
    ...
    if (response.data.success === 1) {
      ...
      if (productDetails.length > 0) {
        ...
        if (product.discount_price && product.discount_price !== "0") {
          subtotal = parseFloat(product.discount_price) * parseInt(product.count || 1);
        }
      }
      ...
      if (subtotal !== parseFloat(discounted_total || "0")) {
        console.warn("API subtotal doesn't match cart calculation. Using cart value.");
  ...
  //     if (selectedCoupon) {
  //       params.offer_code = selectedCoupon.id;
  //     }

  //     console.log(`this is coupon code. ${JSON.stringify(selectedCoupon)} : ${JSON.stringify(params)}`);
  ...
  //       Alert.alert(
  //         "Order Confirmed",
  //         "Your order has been placed successfully! Confirmation will be provided within 12 hours.",
  //         [
  //           {
  //             text: "View Invoice",
  //             onPress: () => {
...
{customerBalance > 0 && (
          <View style={styles.invoiceRow}>
            ...
            <Text style={styles.invoiceNegative}>- ₹{customerBalance.toFixed(2)}</Text>
          </View>
        )}
        {adminBalance > 0 && (
          <View style={styles.invoiceRow}>
            <Text style={styles.invoiceLabel}>Balance Amount</Text>
            <Text style={styles.invoicePositive}>+ ₹{adminBalance.toFixed(2)}</Text>
          </View>
        )}
    ...
    <View style={styles.modalBox}>
      ...
      {selectedPaymentType === "cheque" && (
        <View style={styles.paymentInstructions}>
          ...
          <Text style={styles.instructionsText}>
            Your order will be processed after we receive and clear the cheque.
          </Text>
        </View>
      )}
